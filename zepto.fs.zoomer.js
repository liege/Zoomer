...
;(function ($, window) {
	...
	function _build($target, data) {
		if (!$target.data("zoomer")) {
			...
			if (data.controls.zoomIn || data.controls.zoomOut || data.controls.next || data.controls.previous) {
				...
				var controls = '<div class="zoomer-controls zoomer-controls-' + data.controls.position + '">';
				controls += '<span class="zoomer-previous">&lsaquo;</span>';
				controls += '<span class="zoomer-zoom-out">-</span>';
				controls += '<span class="zoomer-zoom-in">+</span>';
				controls += '<span class="zoomer-next">&rsaquo;</span>';
				controls += '</div>';


				// data.$zoomer.append(controls);
				$(".zoomer_wrapper").append(controls);
				
				var imgBox = '<div class="zoomer-positioner">';
					imgBox += '<div class="zoomer-holder">';
					imgBox += '</div>';
					imgBox += '</div>';
					
				$(".zoomer_wrapper").css({position:"relative"});
				//@liege next&prev box
				$(".zoomer_wrapper").append('<div id="next" class="zoomer"></div>');
				var ww = $(".zoomer_wrapper").width();
				$("#next").css({position:"absolute",left:ww,top:0});
				$("#next").append(imgBox);
				
				$(".zoomer_wrapper").append('<div id="prev" class="zoomer"></div>');
				$("#prev").css({position:"absolute",left:-ww,top:0});
				$("#prev").append(imgBox);
				...
				data.controls.$default = $(".zoomer_wrapper").find(".zoomer-controls");
				data.controls.$zoomIn = $(".zoomer_wrapper").find(".zoomer-zoom-in");
				data.controls.$zoomOut = $(".zoomer_wrapper").find(".zoomer-zoom-out");
				data.controls.$next = $(".zoomer_wrapper").find(".zoomer-next");
				data.controls.$previous = $(".zoomer_wrapper").find(".zoomer-previous");
			}
	...
	function _onImageLoad(e) {
		...
		//@liege next&prev 宽高
		$("#next .zoomer-holder").css({
			height: data.naturalHeight,
			width:	data.naturalWidth
		});
		$("#prev .zoomer-holder").css({
			height: data.naturalHeight,
			width:	data.naturalWidth
		});
		...
		if (transformSupported) {
			...
			//@liege 计算缩放比例
			// alert(data.imageWidth);
			// alert($("#next .zoomer-holder").find("img")[0]);
			// $("#next .zoomer-holder").css( _prefix("transform", "translate3d(-50%, -50%, 0) scale("+scaleX+","+scaleY+")") );
		} else {
			data.$positioner.css({
		...
		$("#next .zoomer-holder").html("");
		if(data.source[data.index-1] != "undefined"){
			$("#next .zoomer-holder").append('<img id="nextImg" src='+data.source[data.index-1]+'>');
			
		}
		$("#prev .zoomer-holder").html("");
		if(data.source[data.index+1] != "undefined"){
			$("#prev .zoomer-holder").append('<img id="prevImg" src='+data.source[data.index+1]+'>');
		...
		for (var i = 0, count = $instances.length; i < count; i++) {
			...
			if (typeof data === "object") {
				...
				if (transformSupported) {
					var scaleX = data.imageWidth / data.naturalWidth,
						scaleY = data.imageHeight / data.naturalHeight;
					//console.log(data.naturalWidth+"--"+data.naturalHeight);
					data.$positioner.css(_prefix("transform", "translate3d(" + data.positionerLeft + "px, " + data.positionerTop + "px, 0)"));
					data.$holder.css(_prefix("transform", "translate3d(-50%, -50%, 0) scale(" + scaleX + "," + scaleY + ")"));
					...
					if(!data.liege){
						// console.log("==>"+$("#prev img").width()/data.naturalWidth);
					}
					// data.liege = true;
					$("#next .zoomer-positioner").css(_prefix("transform", "translate3d(" + data.positionerLeft + "px, " + data.positionerTop + "px, 0)"));
					$("#next .zoomer-holder").css(_prefix("transform", "translate3d(-50%, -50%, 0) scale(" + scaleX + "," + scaleY + ")"));
					$("#prev .zoomer-holder").css(_prefix("transform", "translate3d(-50%, -50%, 0) scale(" + scaleX + "," + scaleY + ")"));
					$("#prev .zoomer-positioner").css(_prefix("transform", "translate3d(" + data.positionerLeft + "px, " + data.positionerTop + "px, 0)"));
				} else {
					data.$positioner.css({
						left: data.positionerLeft,
						top: data.positionerTop
	...
	function _nextImage(e) {
		...
		if(data.source[data.index+1]){
		$(".zoomer-gallery").animate({left:data.frameWidth},300,function(){
			$(".zoomer-gallery").css({left:0});	
		...
		$("#prev").animate({left:0},300,function(){
			$("#prev").css({left:-data.frameWidth});
	...
	function _onTouchStart(data) {
		...
		data.touchStartX = data.touches[0].pageX;
	...
	function _onTouchMove(data) {
		if (data.touches.length === 1) {
			...
			if (data.pinchEndX0 !== data.lastPinchEndX0 || data.pinchEndY0 !== data.lastPinchEndY0 ||
				...
				$('#info').html('width:'+ data.targetImageWidth+',height:'+data.targetImageHeight);
	...
	function _onTouchEnd(data, oe) {				
		if(data.action === 'drag'){
			// console.log(data.mouseX - data.touchStartX);
			if(data.mouseX - data.touchStartX  > 50 && canSlider){
				// console.log('trigger');
				_nextImage(oe);   //@liege
				// console.log("data next");
				data.controls.$next.trigger("click.zoomer");
			}
			if( data.mouseX - data.touchStartX < -50 && canSlider){
				_previousImage(oe);  //@liege
				// console.log("data prev");
				data.controls.$previous.trigger("click.zoomer");
			}
		}
		data.action = "";
